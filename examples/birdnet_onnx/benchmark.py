...
* A **species label file is mandatory**. Unlike DenseNet -- where Triton
  ...
  top-k client-side, so the client maps indices to names itself. The names must
  match the ground truth byte-for-byte.
* **The v3.0 graph applies the sigmoid itself**, so its output is already a
  probability and nothing is applied on top. Confidences are directly comparable
  to the reference package's, and ``confidence_thres`` behaves as documented.
...
if __name__ == "__main__":
    ...
    # config.pbtxt pins cudnn_conv_algo_search so repeat runs are bit-identical;
    # without it Triton auto-completes and the default EXHAUSTIVE search makes
    # every model load pick kernels afresh. See the file's own comments.
    ...
    inference_client = TritonBirdNetClient(
        ...
        # segments, no overlap. The sigmoid lives in the model graph.
        sample_rate=32_000,
        segment_seconds=3.0,
        overlap_seconds=0.0,
        top_k=TOP_K,
        # Matches the reference's default_confidence_threshold. This only began
        # filtering once the double sigmoid was removed -- before that every
        # score sat above 0.5, so any threshold below 0.5 was a no-op.
    ...
    # ------------------------------------------------------------ local scoring
    ground_truth_path = Path("annotations.csv")
