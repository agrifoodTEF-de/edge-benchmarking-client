...
diverged (resampler, bandpass, or segment boundaries -- the v3.0 sigmoid lives
in the model graph and is applied by neither side); differing *sets* usually
means a parameter is not actually matched.

This is a *weak* check on its own: it compares two top-k lists after a lossy
projection of an 11 560-dim vector, so it passes for any defect that preserves
ranking. The double sigmoid (birdnet #86) was exactly such a defect and this
script reported MATCH throughout, because both sides shared it.
...
Usage:
    ...
    cd examples
    python verify_against_reference.py path/to/clip.wav
"""
...
def reference_rows(wav: Path) -> dict:
    ...
    ``load_custom`` rather than ``load``: the latter downloads birdnet's own
    copy of the v3.0 ONNX, which is a *different file* from the ``model.onnx``
    staged here and served to the device (measured max |delta| 5e-4 on identical
    input -- same rankings, but a confound sitting inside the 0.01 tolerance
    this script checks). Both sides must run the same weights, or a preprocessing
    difference and a model difference are indistinguishable in the result.
    """
    import birdnet
    ...
    model = birdnet.load_custom(
        ...
        "3.0",
        "onnx",
        Path(EXAMPLE_ROOT_DIR) / "model.onnx",
        Path(EXAMPLE_ROOT_DIR) / "labels.txt",
        precision=PRECISION,
    )
...
def benchmark_rows(wav: Path) -> dict:
    ...
    # Pins the convolution algorithm search; without it the device's own
    # run-to-run variance (up to 3.5e-04) is folded into this comparison.
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.exit("usage: python verify_against_reference.py <clip.wav>")
